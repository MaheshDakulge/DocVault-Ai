...
async def scan_document(
    ...
    stored_payload = json.dumps({
        "storage_path": storage_path,   # ✅ snake_case
        "file_hash": file_hash,          # ✅ snake_case
        **gemini_result,
    })
...
async def confirm_scan(
    ...
    # ✅ Use snake_case keys matching the updated DB schema
    storage_path = gemini_data.pop("storage_path", None)
    file_hash    = gemini_data.pop("file_hash", None)
    category     = body.category or gemini_data.get("category", "Other")

    # ── Sanitize date fields (must be YYYY-MM-DD or None for date columns) ────
    ...
    doc_row = {
        ...
        "storage_path":   storage_path,       # ✅ was "storagePath"
        "file_hash":      file_hash,           # ✅ was "fileHash"
        "category":       category,
        "subcategory":    gemini_data.get("subcategory"),
        "doc_type":       gemini_data.get("doc_type"),
        "document_date":  _safe_date(gemini_data.get("document_date")),
        "expiry_date":    _safe_date(gemini_data.get("expiry_date")),
    ...
    logger.info("Inserting doc_row: %s", doc_row)  # helpful for debugging

    try:
        doc_result = supabase.table("documents").insert(doc_row).execute()
    except Exception as exc:
        logger.error("Document insert failed: %s", exc)
    ...
    field_rows = [
        {
            "document_id": doc_id,      # ✅ was "document_id" (matches renamed column)
            "label":       f.label,
            "value":       f.value,
            "confidence":  f.confidence,
            "is_copyable": f.is_copyable if hasattr(f, "is_copyable") else True,  # ✅ was "isCopyable"
            "created_at":  now,
        }
        for f in raw_fields
    ]
    ...
    logger.error("doc keys from DB: %s", list(doc.keys()))
    logger.error("DocumentResponse fields: %s", list(DocumentResponse.model_fields.keys()))
    try:
        safe_doc = {k: doc.get(k) for k in DocumentResponse.model_fields if k != "fields"}
        logger.error("safe_doc built: %s", safe_doc)
        return DocumentResponse(**safe_doc, fields=field_items)
    except Exception as exc:
        logger.error("DocumentResponse build failed: %s", exc)
        raise HTTPException(status_code=500, detail=f"Response build error: {exc}")
    safe_doc = {k: doc.get(k) for k in DocumentResponse.model_fields if k != "fields"}
    return DocumentResponse(**safe_doc, fields=field_items)
